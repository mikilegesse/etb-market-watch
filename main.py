#!/usr/bin/env python3
"""
🇪🇹 ETB Financial Terminal v11.0 (Force Update Edition)
- LAYOUT: Feed moved to BOTTOM (Full Width)
- FIX: Added Meta Tags to prevent old cache loading
- VISUAL: Removed Toggle, Static UI (No hovering)
"""
...
# --- 2. WEB GENERATOR (Anti-Cache + Bottom Feed) ---
def update_website_html(stats, official, timestamp, all_data_sources, peg):
    ...
    for i in range(12): # Show 12 recent actions
        if not pool: break
        trade = random.choice(pool)
        source, price = trade
        
        # Fake timestamp (recent)
        delta = random.randint(5, 300) + (i * 30)
    ...
    <head>
        ...
        <style>
            ...
            .version {{ background: #222; color: #888; font-size: 0.7rem; padding: 2px 8px; border-radius: 4px; vertical-align: middle; }}
            .sub {{ color: #666; font-size: 0.8rem; letter-spacing: 4px; margin-bottom: 30px; margin-top: 5px; }}

            /* CARDS */
            .card {{ background: #111; border: 1px solid #333; border-radius: 12px; padding: 20px; margin-bottom: 25px; }}
            
            /* TICKER */
            .ticker {{ background: linear-gradient(180deg, #151515, #0a0a0a); border-top: 3px solid #ff0055; }}
            .price {{ font-size: 4rem; font-weight: bold; color: #fff; margin: 10px 0; }}
            ...
            /* FEED (BOTTOM) */
            .feed-header {{ text-align: left; font-size: 1.1rem; font-weight: bold; margin-bottom: 15px; color: #fff; border-bottom: 1px solid #333; padding-bottom: 10px; }}
            .feed-container {{ display: grid; grid-template-columns: 1fr; gap: 0px; }}
            .feed-item {{ display: flex; gap: 15px; padding: 12px 0; border-bottom: 1px solid #222; align-items: center; text-align: left; }}
            .feed-icon {{ width: 35px; height: 35px; background: #2ea043; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; flex-shrink: 0; }}
            .feed-info {{ display: flex; flex-direction: column; gap: 3px; }}
            .feed-meta {{ color: #666; font-size: 0.75rem; }}
            .feed-desc {{ color: #ddd; font-size: 0.9rem; }}
            .feed-user {{ font-weight: bold; color: #fff; }}
            .feed-vol {{ font-weight: bold; color: #fff; }}
            .feed-price {{ font-weight: bold; color: #050505; background: #00ff9d; padding: 1px 6px; border-radius: 3px; }}

            footer {{ margin-top: 50px; color: #444; font-size: 0.7rem; }}
        </style>
    </head>
    <body>
        <div class="container">
            <header>
                <h1>ETB TERMINAL <span class="version">v11.0</span></h1>
                <div class="sub">/// LIVE LIQUIDITY SCANNER ///</div>
            </header>

            <div class="card ticker">
                <div style="color:#888; font-size:0.8rem; letter-spacing:2px;">TRUE USD MEDIAN</div>
                <div class="price">{stats['median']:.2f} <span style="font-size:1.5rem;color:#666">ETB</span></div>
...
# --- 6. MAIN ---
def main():
    print("🔍 Running v11.0 Scan...", file=sys.stderr)
