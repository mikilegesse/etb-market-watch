#!/usr/bin/env python3
"""
🇪🇹 ETB Financial Terminal v30.2 (FIXES)
- FIX: Sorted prices in analyze() to fix min/max swap
- FIX: Improved MEXC error handling with debug output
- FIX: Bybit excluded from transaction history (no API)
...
def fetch_p2p_army_ads(market, side):
    """ Uses your API Key for MEXC/Binance with improved error handling """
    url = "https://p2p.army/v1/api/get_p2p_order_book"
    h = HEADERS.copy(); h["X-APIKEY"] = P2P_ARMY_KEY
    try:
        print(f"   > Fetching {market.upper()}...", file=sys.stderr)
        r = requests.post(url, headers=h, json={"market":market,"fiat":"ETB","asset":"USDT","side":side,"limit":100}, timeout=15)
        
        # Debug: Print status code
        print(f"   > {market.upper()} Status: {r.status_code}", file=sys.stderr)
        
        if r.status_code != 200:
            print(f"   ⚠️ {market.upper()} API returned {r.status_code}", file=sys.stderr)
            return []
        
        data = r.json()
        
        # Try multiple possible response structures
        raw = (data.get("result", {}).get("data", {}).get("ads", []) or 
               data.get("data", {}).get("ads", []) or 
               data.get("ads", []))
        
        if not raw:
            print(f"   ⚠️ {market.upper()} returned no ads. Response keys: {list(data.keys())}", file=sys.stderr)
            return []
        ...
        for ad in raw:
            try:
                ...
                print(f"   ⚠️ Failed to parse {market.upper()} ad: {e}", file=sys.stderr)
                continue
        
        print(f"   ✅ {market.upper()}: {len(clean)} ads", file=sys.stderr)
        return clean
...
def capture_market_snapshot():
    ...
    with ThreadPoolExecutor(max_workers=10) as ex:
        ...
        return bin_data + byb_data + mexc_data
...
def detect_real_trades(current_ads, peg):
    ...
    ⚠️ EXCLUDES BYBIT (no reliable API for trade detection)
    """
    prev_state = load_market_state()
    trades = []
    
    for ad in current_ads:
        # 🚫 Skip Bybit for trade detection
...
def update_website_html(stats, official, timestamp, actions, grouped_ads, peg):
    ...
    if actions:
        ...
        for item in actions[:25]:
            s_col = "#f3ba2f" if "Binance" in item['source'] else "#2e55e6"  # Removed Bybit color
            feed_html += f"""
            <div class="feed-item">
    ...
    <body>
        <div class="container">
            ...
            <div class="right-col">
                <div class="card">
                    <div class="feed-title">📢 Real-Time Trades (Binance + MEXC Only)</div>
                    <div class="feed-container">{feed_html}</div>
                </div>
            </div>

            <footer>Official Bank Rate: {official:.2f} ETB | Last Update: {timestamp} UTC | 🚫 Bybit excluded from trade history</footer>
        </div>
...
# --- 7. MAIN ---
def main():
    print("🔍 Running v30.2 Fixed Scan...", file=sys.stderr)
