#!/usr/bin/env python3
"""
🇪🇹 ETB Financial Terminal v34.0 (SMART PARSER)
- FEATURE: "Deep Search" logic to find Volume/Name even if API keys change.
- DEBUG: Prints the RAW structure of the first ad to console for verification.
- FIX: Solves the "0 USDT" and "User" display bug.
"""
...
def safe_get(data, keys, default=None):
    """Recursively searches for the first matching key in a nested dict."""
    if isinstance(data, dict):
        for k in keys:
            if k in data and data[k] is not None:
                return data[k]
        # If not found, look deeper (1 level)
...
def parse_ad(raw_ad, source):
    """Intelligently extracts Price, Volume, and User from unknown structures."""
    # Candidate keys for different APIs
    price_keys = ['price', 'unit_price', 'rate']
    vol_keys = ['surplusAmount', 'tradableQuantity', 'available_amount', 'amount', 'dynamicMaxSingleTransAmount', 'maxAmount', 'quantity', 'stock', 'available']
    user_keys = ['nickName', 'nickname', 'advertiser_name', 'name', 'userName']

    try:
        price = float(safe_get(raw_ad, price_keys, 0))
        vol = float(safe_get(raw_ad, vol_keys, 0))
        
        # User name is often nested in an 'advertiser' object
        user = safe_get(raw_ad, user_keys, "Trader")
        if user == "Trader":
             # Try looking specifically inside common nested objects
             if 'advertiser' in raw_ad:
                 user = safe_get(raw_ad['advertiser'], user_keys, "Trader")
...
def fetch_binance_direct(trade_type):
    ...
    while len(ads) < MAX_ADS_PER_SOURCE:
        try:
            ...
            # DEBUG: Print structure of first ad to see what's wrong
            if first_run and data:
                print(f"\n🐛 [DEBUG] RAW BINANCE AD DATA:\n{str(data[0])[:500]}...\n", file=sys.stderr)
                first_run = False
...
def fetch_p2p_army_ads(market, side):
    ...
    try:
        ...
        # DEBUG: Print structure of first P2P Army ad
        if raw:
             print(f"\n🐛 [DEBUG] RAW {market.upper()} AD DATA:\n{str(raw[0])[:300]}...\n", file=sys.stderr)
...
def capture_market_snapshot():
    with ThreadPoolExecutor(max_workers=10) as ex:
        ...
        # If P2P Army fails for Binance, try direct
        if not bin_data: 
            print("   ⚠️ P2P Army empty, using Binance Direct...", file=sys.stderr)
    ...
    for ad in snapshot_after:
        ...
        if key in prev_state:
            ...
            if curr_inventory < prev_inventory:
                ...
                # FILTER: Only show meaningful trades (> 5 USD) to avoid noise if API glitches
                if diff >= 5:
                    trades.append({
...
def update_website_html(stats, official, timestamp, current_ads, grouped_ads, peg):
    ...
    if recent_trades:
        ...
        # FALLBACK: Sort by Lowest Price first (Best Offers)
        ...
        if all_offers:
            ...
            for offer in all_offers[:30]:
                try:
                    ...
                    s_col = "#f3ba2f" if "Binance" in source else "#2e55e6"
                    user_display = offer.get('advertiser', 'Unknown')[:15]
                    
                    # VISUAL FIX: If volume is 0, hide it or show N/A
                    vol_val = offer.get('available', 0)
                    vol_str = f"{vol_val:,.0f} USDT" if vol_val > 0 else "Liquid"
                    
                    feed_html += f"""<div class="feed-item"><div class="feed-icon" style="background:{s_col}">🟡</div><div class="feed-content"><span class="feed-source" style="color:{s_col}">{source}</span>: <span class="feed-user">{user_display}</span> offering <span class="feed-vol">{vol_str}</span> @ <span class="feed-price">{offer['price']/peg:.2f} ETB</span></div></div>"""
                except: continue
        else:
    ...
    <head>
        ...
        <title>ETB Market Watch v34.0</title>
        <style>
...
# --- 6. MAIN ---
def main():
    print("🔍 Running v34.0 (SMART PARSER)...", file=sys.stderr)
