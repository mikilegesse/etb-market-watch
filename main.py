Here it is again, full script in one piece:

#!/usr/bin/env python3
"""
🇪🇹 ETB Financial Terminal v34.0 (Lifecycle + Inventory Tracking - EthioBlackMarket Method)
- NEW REQUEST: Ad ID appears → "a new request"
- BOUGHT: Inventory drops OR Ad ID disappears → "bought remaining"
- SIMPLE: Track IDs + inventory (same spirit as ethioblackmarket.com)
"""
...
def detect_lifecycle_events(snapshot_before, snapshot_after, peg):
    """
    ✅ INVENTORY TRACKING + LIFECYCLE
    - Ad ID appears  -> "new_request"
    - Inventory drops OR ID disappears -> "bought"
    Only Binance + MEXC are tracked.
    """
    if not snapshot_before:
        print("   > First run - establishing baseline", file=sys.stderr)
        return []

    # Index ads by ID for Binance + MEXC only
    def index_ads(ads):
        idx = {}
        for ad in ads:
            src = ad.get("source", "").lower()
            ad_id = ad.get("id")
            if src not in ["binance", "mexc"] or not ad_id:
                continue
            existing = idx.get(ad_id)
            # keep the one with the largest inventory
            if not existing or float(ad.get("available", 0)) > float(existing.get("available", 0)):
                idx[ad_id] = ad
        return idx
    ...
    new_ids    = curr_ids - prev_ids     # new ads appeared
    gone_ids   = prev_ids - curr_ids     # ads disappeared
    common_ids = prev_ids & curr_ids     # ads stayed, inventory may change

    print(
        f"   > Lifecycle: {len(new_ids)} new, {len(gone_ids)} gone, {len(common_ids)} still active",
        file=sys.stderr
    )
    ...
    # 1) NEW REQUESTS (IDs that appeared)
    for ad_id in new_ids:
        ad = curr_ads[ad_id]
        avail = float(ad.get("available", 0))
        if avail > 10:  # ignore tiny noise
            events.append({
                "type": "new_request",
                "source": ad["source"],
                "user": ad["advertiser"],
    ...
    # 2) BOUGHT (IDs that stayed, but inventory dropped)
    for ad_id in common_ids:
        ...
        if new_inv < old_inv:
            ...
            if diff > 50:  # threshold to avoid spam on tiny changes
                events.append({
                    "type": "bought",
                ...
                print(
                    f"   ✅ BOUGHT (partial): {new_ad['source']} - {new_ad['advertiser'][:15]} "
                    f"{diff:,.0f} USDT @ {new_ad['price']/peg:.2f} ETB",
                    file=sys.stderr
                )

    # 3) BOUGHT REMAINING (IDs that disappeared completely)
...
def update_website_html(stats, official, timestamp, current_ads, grouped_ads, peg):
    ...
    # Feed (lifecycle + inventory events!)
    feed_html = ""
    ...
    <body>
        <div class="container">
            ...
            <footer>
                ...
                Tracking ad lifecycle (appearing/disappearing + inventory)
            </footer>
        </div>
...
# --- 7. MAIN (LIFECYCLE + INVENTORY TRACKING) ---
def main():
    ...
    # 2. WAIT
    print(f"   > ⏳ Waiting {BURST_WAIT_TIME}s for ads to change...", file=sys.stderr)
